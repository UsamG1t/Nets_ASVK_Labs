# Лабораторная работа №8 — Маршрутизация сетей с использованием RIP
 ...
 + [Предыдущая лабораторная работа — Статические маршруты и маршруты по умолчанию](../07_StaticRouting/Статические%20маршруты%20и%20маршруты%20по%20умолчанию.html)
 + [Следующая лабораторная работа — Маршрутизация сетей с использованием OSPF](../09_OSPF_Routing/Маршрутизация%20сетей%20с%20использованием%20OSPF.html)
...
# Быстрый поиск
 ...
 + [Пример настройки RIP](#пример-настройки-rip)
   + [Базовая настройка виртуальных машин](#базовая-настройка-виртуальных-машин)
   + [BIRD Routing Daemon](#bird-routing-daemon)
   + [Настройка RIP](#настройка-rip)
...
ℹ Вместе с описанием IP-адресов в таблице маршрутизации автоматически появятся записи о маршрутах в сетях описанных адресов.
...
ℹ Для работы демона необходимо указывать ключевые параметры конфигурации:
...
ℹ Заметьте, что `R2` не будет принимать данные, связанные с сетью 10.0.1.0/24, и, соответственно, не будет отправлять их далее по топологии.

📍 3. С помощью команды `bird` запустите BIRD на _каждом_ из устройств

ℹ C помощью команды `birdc` можно посмотреть параметры работы демона.
...
ℹ В таблице маршрутизации после запуска должны появиться новые записи о доступных маршрутах с указанием `proto bird` , означающим, что маршрут получен с помощью BIRD-демона (поскольку маршруты не приходят мгновенно, может потребоваться время на получение всех данных).
...
ℹ Заметьте, что информация о маршруте в сеть 10.0.1.0/24 не передалась на `R2` и `R3`, поскольку не была указана, как допустимая к передаче
