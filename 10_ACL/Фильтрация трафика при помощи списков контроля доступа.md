# Лабораторная работа №10 — Фильтрация трафика при помощи списков контроля доступа
 ...
 + [Предыдущая лабораторная работа — Маршрутизация сетей с использованием OSPF](../09_OSPF_Routing/Маршрутизация%20сетей%20с%20использованием%20OSPF.html)
 + [Следующая лабораторная работа — VPN и туннелирование](../11_WG_IPIP/VPN%20и%20туннелирование.html)
...
# Быстрый поиск
 ...
 + [Пример работы со списками контроля доступа](#пример-работы-со-списками-контроля-доступа)
   + [Базовая настройка виртуальных машин](#базовая-настройка-виртуальных-машин)
   + [Настройка маршрутизации в сети](#настройка-маршрутизации-в-сети)
   + [Настройка списков контроля доступа](#настройка-списков-контроля-доступа)
...
📍 2. С помощью команды `bird` запустите BIRD на _каждом_ из устройств. С помощью команд управления таблицами маршрутизации убедитесь, что данные о маршрутах успешно добавились в таблицы маршрутизации R-абонентов.
...
ℹ На данный момент никаких ограничений в сети нет, доступ открыт всем абонентам сети для всех операций в сети.

ℹ Для наглядности и различия действий будет рассматриваться два вида соединений — HTTP-запрос на загрузку данных и SSH-подключение к абонентам.

📍 1. С помощью команды встроенного модуля Python `python3 -m http.server`  запустите на S-узлах HTTP-серверы
...
ℹ Для проверки не только IP-связности, но и доступности сервисов будут использоваться команды wget и ssh:
...
📍 2. С помощью команд `wget <dstIP>:<dstPort>` и `ssh <dstIP>` на R1 и R3 проверьте доступность сервисов S-абонентов
...
ℹ Как видно из показаний серверов, оба соединения к каждому из абонентов прошли успешно:
...
ℹ Теперь настроим следующие ограничения доступа:
...
ℹ Настройку списков контроля доступа осуществим с помощью команд [`ip rule`](https://man7.org/linux/man-pages/man8/ip-rule.8.html), позволяющих манипулировать правилами обращения к таблицам маршрутизации. Правила будут задаваться на промежуточном узле R2
...
ℹ Для правил в списке контроля доступа может задаваться приоритет. Правила с меньшим значением `priority` имеют приоритет над полями с большим значением. Принцип выбора срабатывания правил — сверху вниз до первого совпадения параметров.
...
📍 6. С помощью команд мониторинга сети убедитесь в наличии связности R1 и R3 c R2 и отсутствии связности с S-абонентами
...
📍 7. С помощью команд `wget <dstIP>:<dstPort>` и `ssh <dstIP>` на R1 и R3 проверьте доступность разрешённых сервисов S-абонентов и запрет остальных.
...
📍 Полученные отчёты `report.10.r1`, `report.10.r2`, `report.10.s1`, `report.10.s2` через последовательный порт перенести из виртуальной машины и прислать их преподавателю с подписью выполненного варианта.
