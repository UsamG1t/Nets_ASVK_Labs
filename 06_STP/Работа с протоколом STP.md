# Лабораторная работа №6 — Работа с протоколом STP
 ...
 + [Предыдущая лабораторная работа — Маршрутизация сетей с VLAN](../05_VLAN_Routing/Маршрутизация%20сетей%20с%20VLAN.html)
 + [Следующая лабораторная работа — Статические маршруты и маршруты по умолчанию](../07_StaticRouting/Статические%20маршруты%20и%20маршруты%20по%20умолчанию.html)

---

# Быстрый поиск

 + [Протокол STP](#протокол-stp)
 + [Пример настройки STP](#пример-настройки-stp)
   + [Настройка коммутаторов](#настройка-коммутаторов)
   + [Параметры сетевого моста](#параметры-сетевого-моста)
   + [Работа с STP](#работа-с-stp)
...
📍 2. C помощью команд управления интерфейсами включите на коммутаторах все используемые интерфейсы
...
ℹ В работе протокола STP учитываются несколько параметров данного сетевого моста:
...
ℹ Работа протокола подразумевает постоянную передачу служебных сообщений между коммутаторами ([BPDU](https://ru.wikipedia.org/wiki/BPDU)) для синхронизации состояний. За скорость синхронизации отвечают параметры:
...
ℹ На данный момент в настраиваемых нами системах протокол STP отключён. Оба коммутатора считают себя единственными (и потому корневыми) коммутаторами в системе.
...
📍 2. С помощью команды управления интерфейсами выведите информацию о параметрах сетевого моста. Обратите внимание на параметры stp_state, priority, bridge_id, designated_port, root_port
...
📍 3. С помощью _команды изменения параметров интерфейса_ :new: `ip link set dev br0 type bridge stp_state 1` на коммутаторах включите протокол STP. С помощью команд управления интерфейсами и сетевым мостом выведите параметры интерфейсов и моста:
...
ℹ Поскольку приоритеты коммутаторов одинаковы, корневым автоматически становится коммутатор с меньшим `bridge_id`. У зависимого коммутатора при этом меняются значения `designated_root` и `root_port`.
...
ℹ Для смены корневого коммутатора можно изменить приоритет.

📍 4. С помощью команды изменения параметров интерфейса на _зависимом_ коммутаторе установите новое значение приоритета моста.
...
ℹ После синхронизации первый коммутатор стал корневым, а второй — зависимым.

ℹ Для управления каналами передачи данных используются настройки приоритета портов и «стоимости» каналов.

📍 5. С помощью команды изменения параметров интерфейса на _корневом_ коммутаторе установите новое значение приоритета связанного порта eth1.
...
ℹ На _зависимом_ коммутаторе поменяются статусы каналов, активным станет канал с меньшим значением `priority` у соседа.
...
📍 6. С помощью команды изменения параметров интерфейса на _зависимом_ коммутаторе установите новое значение стоимости канала связанного порта eth2.

ℹ Статусы каналов вновь поменяются, использоваться будет канал с меньшей стоимостью
...
📍 Полученные отчёты `report.06.r1`, `report.06.r2`, `report.06.r3`, `report.06.r4`, `report.06.r5` через последовательный порт перенести из виртуальной машины и прислать их преподавателю с подписью выполненного варианта.
